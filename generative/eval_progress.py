...
def load_history(pdf_filter: str | None = None, version_filter: str | None = None) -> list[dict]:
...
def print_table(records: list[dict]) -> None:
    ...
    for r in sorted(records, key=lambda x: (x.get("version", ""), x.get("timestamp", ""))):
        ...
        print(
            f"{ts:<17} {v:<8} {ev:<5} {lang:>6} {note:<36} "
    ...
    print(
        f"{'GESAMT':<17} {'':<8} {'':<5} {'':<6} {f'{len(records)} Notes':<36} "
