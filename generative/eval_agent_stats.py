        ...
        stats.setdefault(agent, {}).update(
            {
                ...
                "avg_duration_ms": (
                    int(sum(c.get("duration_ms", 0) for c in not_cached) / len(not_cached)) if not_cached else 0
                ),
            }
        )

    for agent, rates in anchor_rates.items():
        stats.setdefault(agent, {})["avg_confirmation_rate"] = round(sum(rates) / len(rates), 3) if rates else 0.0

    for agent, sc in scores.items():
        stats.setdefault(agent, {})["avg_score"] = round(sum(sc) / len(sc), 2) if sc else 0.0
...
def _print_table(stats: dict, model_config: dict | None, run_id: str) -> None:
    ...
    print()
    print(
        f"{'Agent':<18} {'Calls':>6} {'Err%':>6} {'In-Tok':>9} {'Out-Tok':>8} {'avg ms':>7} {'avg Score':>9} {'Anker%':>8} {'Vault%':>7}"
    )
    print("-" * 82)
