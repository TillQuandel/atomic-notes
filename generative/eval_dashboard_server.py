...
def _read_calibration_data(allowed_note_paths: set | None = None, eval_version: str = "4.1") -> dict:
...
def build_data(
    ...
    if pipeline_version:
        quality_rows = [r for r in quality_rows if (r.get("version") or r.get("pipeline_version")) == pipeline_version]
    if language:
    ...
    try:
        ...
        print(f"[db-only-runs] Fehler: {_e}", file=_sys.stderr)
    ...
    if _exclude_foss:
        ...
        log_data = D._build_log_data(all_log_runs)
        quality_rows = [r for r in quality_rows if not D.is_foss_version(r.get("version") or r.get("pipeline_version"))]
    ...
    for r in all_log_runs:
        ver = r.get("ver") or "?"
        accept_pairs_by_ver.setdefault(ver, []).append((r.get("n_vault", 0) or 0, r.get("n_total", 0) or 0))
    ...
    kpi_trend = {
        ...
        "tokens": [
            round(sum(tok_by_ver.get(v, [])) / 1000, 1) if tok_by_ver.get(v) else None for v in sorted_pipeline_versions
        ],  # in M-Tokens
        "cost": [
...
def _live_run_data() -> dict:
    ...
    agents: dict = defaultdict(
        lambda: {"calls": 0, "cached": 0, "tokens_in": 0, "tokens_out": 0, "dur_ms": 0, "errors": 0}
    )
    ...
    return {
        ...
        "elapsed_s": elapsed_s,
        "agents": dict(agents),
        "total": {"tokens_in": total_in, "tokens_out": total_out, "calls": sum(v["calls"] for v in agents.values())},
    }
