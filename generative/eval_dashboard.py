...
# pipeline_runs.pipeline_version-Marker fuer Re-Eval-Sweeps (reeval_baseline.py):
# 'reeval' fuer normale Sweeps, 'reeval-repeat' fuer --repeat-Wiederholungs-
# Sweeps (eigenes Run-Label, damit die Rauschanalyse Wiederholungsgruppen
# unterscheiden kann, s. reeval_baseline.py). Beide sind fuer diese Funktion
# gleichwertig "reeval-like" -- dieselbe Diagnose (Zeilen messen den
# Eval-Code-Stand des Sweeps, nicht die Notes-Erzeugungsversion) gilt fuer
# Repeat-Sweeps genauso wie fuer normale Re-Eval-Sweeps.
_REEVAL_PIPELINE_VERSIONS = {"reeval", "reeval-repeat"}
...
def _is_reeval_series(quality_rows: list[dict], pipeline_runs: list[dict]) -> bool:
    ...
    Eintrag mit `pipeline_version` in `_REEVAL_PIPELINE_VERSIONS`
    ("reeval"/"reeval-repeat") haengen.
    ...
    Bestaetigung (jede Zeile matcht einen der beiden Marker) schaltet das Flag."""
    if not quality_rows:
        return False
    run_to_pver = {r.get("run_id"): r.get("pipeline_version") for r in pipeline_runs if r.get("run_id")}
    return all(run_to_pver.get(row.get("run_id")) in _REEVAL_PIPELINE_VERSIONS for row in quality_rows)
