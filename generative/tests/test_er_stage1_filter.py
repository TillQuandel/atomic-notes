...
def test_disjoint_concepts_skipped():
    verdict, _ = _decide("Five Laws Library Science", "Information Search Process")
    assert verdict == "skip-no-subset"


def test_partial_overlap_no_subset_skipped():
    # Schnittmenge aber keine Subset-Relation.
    verdict, _ = _decide("Information Search Process", "Information Need Wilson")
    assert verdict == "skip-no-subset"
...
def test_author_suffix_accepted():
    ...
    verdict, diff = _decide("Five Laws Library Science", "Five Laws Library Science Bates")
    assert verdict == "accept"
    assert diff == 1


def test_author_suffix_year_accepted():
    verdict, diff = _decide("ISP Modell Kuhlthau", "ISP Modell Kuhlthau 1991")
    assert verdict == "accept"
    assert diff == 1
...
def test_specific_concept_with_author_accepted():
    ...
    verdict, diff = _decide("Berrypicking Foraging Pattern", "Berrypicking Foraging Pattern Bates")
    assert verdict == "accept"
    assert diff == 1
...
def test_hub_generic_shorter_blocked_even_with_author_suffix():
    ...
    verdict, diff = _decide("Information Behavior Concept", "Information Behavior Concept Bates")
...
def test_hub_sub_two_token_diff_skipped():
    ...
    verdict, diff = _decide("Taylor Information Need Model", "Taylor Vier Stufen Typologie Information Need Model")
    assert verdict == "skip-token-diff"
    assert diff > ER_MAX_TOKEN_DIFF


def test_specialization_three_token_diff_skipped():
    verdict, diff = _decide("Information Behavior Bates", "Information Behavior Bates Berrypicking Foraging Theory")
    assert verdict == "skip-token-diff"
    assert diff >= 3
