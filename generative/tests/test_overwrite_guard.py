...
def _draft(title="Test Note", body="pipeline body"):
    return AtomicNoteDraft(
        title=title, body=body, source_anchors=[], related=[], tags=["t"],
        synthesis_confidence="low", critic_score=5, hard_gates_pass=True)


class TestContentHashHelpers:
    def test_injected_note_is_pristine(self):
        from generative.pipeline.vault_writer import (
            inject_content_hash, is_pristine_pipeline_note)
...
class TestRenderNoteHasHash:
    def test_render_note_embeds_content_hash(self):
        from generative.pipeline.vault_writer import (
            render_note, is_pristine_pipeline_note)
...
class TestHubAndStubGetHash:
    def test_render_moc_embeds_hash(self):
        ...
        hub = AtomicNoteDraft(
            title="MoC Test", body="hub body", source_anchors=[], related=[],
            tags=["t"], synthesis_confidence="low", action="hub",
            hub_subconcepts=["A", "B"], critic_score=5, hard_gates_pass=True)
...
class TestMergeStubOverwriteGuard:
    def test_edited_merge_stub_not_overwritten(self, tmp_path, monkeypatch):
        ...
        vault_note.write_text('---\ntitle: "Test Note"\nsource-file: "other.pdf"\n---\nx\n',
                              encoding="utf-8")
        monkeypatch.setattr(vault_writer, "find_existing_in_vault",
                            lambda *a, **k: vault_note)
        inbox = tmp_path / "inbox"; inbox.mkdir()
        # 1. erster Merge-Stub-Write
        p1 = vault_writer.write_note(_draft(), source_file="src.pdf", dry_run=False,
                                     existing_concepts={"x": "y"}, inbox_dir=inbox)
        assert "MERGE" in p1.name
        # 2. Nutzer editiert den Stub
        p1.write_text(p1.read_text(encoding="utf-8") + "\nUSER-EDIT\n", encoding="utf-8")
        # 3. Re-Run
        p2 = vault_writer.write_note(_draft(), source_file="src.pdf", dry_run=False,
                                     existing_concepts={"x": "y"}, inbox_dir=inbox)
...
class TestReRunOverwriteGuard:
    def test_edited_inbox_note_not_overwritten(self, tmp_path):
        ...
        p1 = vault_writer.write_note(_draft(body="ORIGINAL"), source_file="src.pdf",
                                     dry_run=False, inbox_dir=tmp_path)
        ...
        p2 = vault_writer.write_note(_draft(body="NEUER RERUN BODY"),
                                     source_file="src.pdf", dry_run=False,
                                     inbox_dir=tmp_path)
    ...
    def test_finder_prefers_pristine_match_regardless_of_order(self, tmp_path):
        ...
        pristine.write_text(vault_writer.inject_content_hash(
            '---\ntitle: "T"\nsource-file: "s.pdf"\n---\nclean\n'), encoding="utf-8")
    ...
    def test_repeated_reruns_reuse_pristine_variant_no_churn(self, tmp_path):
        ...
        p1.write_text(p1.read_text(encoding="utf-8").replace("ORIGINAL", "ORIGINAL\nUSER"),
                      encoding="utf-8")
        p2 = vault_writer.write_note(_draft(body="RERUN A"), source_file="src.pdf",
                                     dry_run=False, inbox_dir=tmp_path)  # → Variante
        p3 = vault_writer.write_note(_draft(body="RERUN B"), source_file="src.pdf",
                                     dry_run=False, inbox_dir=tmp_path)  # → pristine Variante überschreiben
        assert p3 == p2
