...
def test_verifier_run_emits_anchor_stats(tmp_path, monkeypatch):
    ...
    draft = AtomicNoteDraft(
        title="Test Note", body="Kurztext.", source_anchors=[],
...
def test_langfuse_backend_write_does_not_crash_without_package(monkeypatch):
    ...
    backend.write({"agent": "extractor", "input_tokens": 100, "output_tokens": 50, "duration_ms": 1000, "cached": False, "ts": "2026-01-01T00:00:01"})
