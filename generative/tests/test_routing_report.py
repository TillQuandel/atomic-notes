...
def _draft(title="T", critic_score=5, hard_gates_pass=True,
           quality_flags=None, source_status=None, action="create",
           hub_subconcepts=None):
    return AtomicNoteDraft(
        title=title, body="b", source_anchors=[], related=[], tags=[],
        synthesis_confidence="low",
        quality_flags=quality_flags or [],
        action=action,
        hub_subconcepts=hub_subconcepts or [],
        critic_score=critic_score, hard_gates_pass=hard_gates_pass,
    ...
    def test_includes_concrete_flags_in_live_run(self):
        ...
        line = routing_status_line(
            _draft(critic_score=3, hard_gates_pass=True,
                   quality_flags=["⚠️ nur 1 Quelle", "⚠️ nicht peer-reviewed"]))
