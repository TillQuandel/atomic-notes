...
def _draft(title: str, action: str = "create") -> AtomicNoteDraft:
    return AtomicNoteDraft(
        title=title, body="", source_anchors=[], related=[], tags=[],
        synthesis_confidence="medium", action=action,
    )


def test_five_shared_token_drafts_suggested():
    drafts = [_draft(t) for t in [
        "Agent Architektur", "Agent Memory", "Agent Loop",
        "Agent Tooling", "Agent Planung",
    ]]
...
def test_four_below_threshold():
    drafts = [_draft(t) for t in [
        "Agent Architektur", "Agent Memory", "Agent Loop", "Agent Tooling",
    ]]
...
def test_stopword_token_not_suggested():
    ...
    drafts = [_draft(t) for t in [
        "Recht oder Gesetz", "Steuer oder Abgabe", "Kauf oder Miete",
        "Lohn oder Gehalt", "Zins oder Rendite",
    ]]
    assert suggest_unmarked_clusters(drafts) == []
...
def test_short_token_not_suggested():
    ...
    drafts = [_draft(t) for t in [
        "AI is Cool", "ML is Hard", "RL is Slow", "NLP is Big", "CV is Old",
    ]]
    assert suggest_unmarked_clusters(drafts) == []
...
def test_strongest_cluster_first():
    ...
    drafts = [_draft(t) for t in [
        "Agent Architektur", "Agent Memory Store", "Agent Memory Cache",
        "Agent Loop", "Agent Tooling",
    ]]
    result = suggest_unmarked_clusters(drafts)
